...
service provides a partial wrapper around `PlayerState` from ClientStructs and
additionally provides the `CurrentWorld` and `HomeWorld` properties of type
`RowRef<World>`.
...
- Fixed `IsClientIdle` returning false when the player is on the Free Trial.

#### IDalamudPluginInterface

- The service now inherits from `IServiceProvider`.
