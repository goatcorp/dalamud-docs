...
invoke callsites in the native game code, this meant that we could be
...
We are now able to provide events for **any** AtkUnitBase virtual functions, as 
such the following events have been added:

PreOpen, PostOpen, PreClose, PostClose, PreShow, PostShow, PreHide, PostHide,
PreMove, PostMove, PreMouseOver, PostMouseOver, PreMouseOut, 
PostMouseOut, PreFocus, PostFocus

Note1: Focus events are not triggered when a window is selected, they are
triggered in response to certain popup windows such as SelectYesno
...
end up recursively calling your listener. I believe this behavior may
...
Secondly, ReceiveEvent messages may be slightly different, the old version of
