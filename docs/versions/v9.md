...
Dalamud v9 is the next major version of Dalamud, to be released together with Patch 6.5. This is a high-level overview of changes. You can see a code diff of all of these changes [here.](https://github.com/goatcorp/dalamud/compare/master...v9)
...
- **.NET Version:** .NET 7.0
  - .NET 8 is due for official release in November 2023. Due to release timing,
    ...
    release, when we can confirm that it is stable on all platforms(Windows, Wine, OS X).
...
* Comprehensive new service to acquire ImGui textures from game icons/textures via `ITextureProvider`, detailed below.
