...
import GfdIconData from '@site/src/components/icons/GfdIconData.json';
...
<details>
  <summary>Expand for a list of available icons (as of Patch <div className="gfd-game-version"></div>)</summary>
...
<thead>
  <tr>
    <th>Id</th>
    {Object.keys(GfdIconData.platforms).map((platform) => {
      const title = GfdIconData.platforms[platform];
      return (
        <th key={platform}>
          <abbr title={`common/font/fonticon_${platform}.tex - ${title}`}>
            {platform}
          </abbr>
        </th>
      );
...
{GfdIconData.icons.map(icon => (

<tr key={icon.id}>
  <td>{icon.id}</td>
  <td>
    <div className={'gfd-icon gfd-icon-ps3 gfd-icon-' + icon.id} />
  </td>
  <td>
    <div className={'gfd-icon gfd-icon-ps4 gfd-icon-' + icon.id} />
  </td>
  <td>
    <div className={'gfd-icon gfd-icon-ps5 gfd-icon-' + icon.id} />
  </td>
  <td>
    <div className={'gfd-icon gfd-icon-lys gfd-icon-' + icon.id} />
  </td>
  <td>
    <div className={'gfd-icon gfd-icon-xinput gfd-icon-' + icon.id} />
  </td>
  <td>
    <div className={'gfd-icon gfd-icon-obe gfd-icon-' + icon.id} />
  </td>
</tr>
...
- \[0] Integer Expression: <a
  href="https://github.com/NotAdam/Lumina/blob/master/src/Lumina/Text/Payloads/LinkPayloadType.cs">LinkMacroPayloadType</a>
